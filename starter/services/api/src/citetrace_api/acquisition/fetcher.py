    ...
    async def fetch(
        ...
        *,
        ...
        async with (
            ...
            else _AsyncClientContextManager(self.client) as client
        ):
            while True:
                ...
                async with client.stream(
                    ...
                    if 300 <= response.status_code < 400:
                        if redirects >= max_redirects:
                            raise UrlValidationError(
                                UrlDenialCode.REDIRECT_LIMIT_EXCEEDED, "Too many redirects"
                            )
                    ...
                    async for chunk in response.aiter_bytes():
                        ...
                        if len(body) > maximum_bytes:
                            raise ValueError(
                                f"Response exceeded maximum size of {maximum_bytes} bytes"
                            )
