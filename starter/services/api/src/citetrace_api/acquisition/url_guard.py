...
class ValidatedRemoteLocation:
    def __init__(
        self, url: str, canonical_host: str, resolved_ips: tuple[str, ...], port: int, scheme: str
    ):
        self.url = url
    ...
    async def validate(self, url: str) -> ValidatedRemoteLocation:
        ...
        if parsed.scheme.lower() not in allowed_schemes:
            raise UrlValidationError(
                UrlDenialCode.SCHEME_NOT_ALLOWED, f"Scheme {parsed.scheme} not allowed"
            )

        # Check credentials
        if parsed.username or parsed.password:
            raise UrlValidationError(
                UrlDenialCode.CREDENTIALS_NOT_ALLOWED, "Credentials not allowed in URL"
            )
        ...
        try:
            ...
            raise UrlValidationError(
                UrlDenialCode.DNS_RESOLUTION_FAILED, f"DNS resolution failed for {parsed.hostname}"
            ) from e
        ...
        for info in addr_info:
            ...
            if (
                ...
                raise UrlValidationError(
                    UrlDenialCode.PRIVATE_ADDRESS, f"IP {ip_str} is not a public address"
                )
        ...
        if not resolved_ips:
            raise UrlValidationError(
                UrlDenialCode.DNS_RESOLUTION_FAILED, f"No valid IPs found for {parsed.hostname}"
            )
