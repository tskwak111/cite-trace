...
def validate_pdf(data: bytes) -> PdfValidationReport:
    ...
    if len(data) > MAX_BYTE_COUNT:
        return PdfValidationReport(
            accepted=False,
            code=PdfValidationCode.BYTE_LIMIT_EXCEEDED,
            page_count=None,
    ...
    if not data.startswith(PDF_MAGIC):
        return PdfValidationReport(
            accepted=False,
            code=PdfValidationCode.INVALID_MAGIC,
            page_count=None,
            extracted_character_count=0,
        )
    try:
        ...
        if reader.is_encrypted:
            return PdfValidationReport(
                accepted=False,
                code=PdfValidationCode.ENCRYPTED,
                page_count=None,
                extracted_character_count=0,
            )
        ...
        if page_count > MAX_PAGE_COUNT:
            return PdfValidationReport(
                accepted=False,
                code=PdfValidationCode.PAGE_LIMIT_EXCEEDED,
        ...
        if total_chars < IMAGE_ONLY_CHAR_THRESHOLD:
            return PdfValidationReport(
                accepted=False,
                code=PdfValidationCode.IMAGE_ONLY_UNSUPPORTED,
                page_count=page_count,
                extracted_character_count=total_chars,
            )
        return PdfValidationReport(
            accepted=True,
            code=PdfValidationCode.ACCEPTED,
            page_count=page_count,
            extracted_character_count=total_chars,
        )
        ...
        return PdfValidationReport(
            accepted=False,
            code=PdfValidationCode.MALFORMED,
            page_count=None,
            extracted_character_count=0,
        )
    except Exception:
