...
async def test_openalex_uses_exact_doi_before_title_search(openalex: OpenAlexProvider) -> None:
    ...
    exact_doi_route = respx.get("https://api.openalex.org/works/doi:10.1000/foundation").respond(json=data)
    title_search_route = respx.get("https://api.openalex.org/works").respond(json={"results": [data]})
