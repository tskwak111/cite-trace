...
class MockParsedDocsRepo:
    async def save_parsed_document(self, doc_data): pass
    async def save_parsed_nodes(self, nodes): pass
    async def save_reference_entries(self, refs): pass
    async def save_citation_clusters(self, clusters): pass
    async def save_citation_anchors(self, anchors): pass
...
async def test_registered_document_reaches_parsed_state(object_store, outbox, parsed_docs_repo):
    ...
    await object_store.put_if_absent(source_object_key(workspace_id, sha256), b"pdf_data", "application/pdf")
    
    parsing_service = StubParsingService(object_store, get_good_tei())
    handler = DocumentSourceRegisteredHandler(object_store, parsing_service, parsed_docs_repo, outbox)
...
def test_get_document_endpoint():
    with TestClient(app) as client:
        ...
        app.state.in_memory_outbox.add_event(
            ...
            doc_id,
            ws_id,
            {"parsed_document_id": "p1", "quality_grade": "a"}
        )
