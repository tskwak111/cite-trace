                    ...
                    if response.status_code == 200:
                        ...
                        if len(content) > self.settings.grobid_max_response_bytes:
                            raise GrobidClientError(
                                f"Response exceeds maximum allowed bytes: {len(content)}"
                            )
                        return GrobidParseResult(
                            tei_xml=content, status="success", timings_ms={"grobid_ms": elapsed_ms}
                        )

                    if response.status_code == 503:
                        if attempt >= max_attempts:
                            raise GrobidClientError(
                                f"GROBID service unavailable after {max_attempts} attempts"
                            )
